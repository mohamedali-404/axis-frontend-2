    ...
    return (
        <>
            ...
            <div style={{
                ...
                backgroundColor: 'rgb(var(--background-start-rgb))', color: 'var(--accent-color)', zIndex: 100, transform: isOpen ? 'translateX(0)' : 'translateX(100%)',
                ...
                <div style={{ padding: 'clamp(1.5rem, 4vw, 2rem)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-color)', gap: '1rem' }}>
                    <h2 style={{ fontSize: 'clamp(1.2rem, 5vw, 1.5rem)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '-0.5px', margin: 0 }}>Your Cart</h2>
                    ...
                    {items.length === 0 ? (
                        ...
                        items.map((item) => (
                            <div key={`${item.id}-${item.size}`} style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
                                <div style={{ width: 90, height: 110, backgroundColor: 'var(--secondary-color)', borderRadius: '12px', overflow: 'hidden' }}>
                                ...
                                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                                    ...
                                    <p style={{ color: 'var(--accent-color)', opacity: 0.6, marginTop: '0.25rem', fontSize: '0.9rem', fontWeight: 500 }}>Size: {item.size}</p>
